...
import commitTimeRaw from './commit_time.txt?raw';
...
function init(): void {
    ...
    // Display commit time in HUD (generated by pre-commit hook)
    const commitTimeDisplay = document.getElementById('commit-time-display');
    if (commitTimeDisplay) {
        const commitTime = commitTimeRaw.trim();
        commitTimeDisplay.textContent = `Commit time: ${commitTime.length > 0 ? commitTime : 'unknown'}`;
    }
